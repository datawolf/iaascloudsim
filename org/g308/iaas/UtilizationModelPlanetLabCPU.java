/**
 * 
 */
package org.g308.iaas;

import java.io.BufferedReader;
import java.io.FileReader;
import java.io.IOException;

import org.cloudbus.cloudsim.UtilizationModel;

/**
 * The Class UtilizationModelPlanetLabCPU
 * 
 * @author wanglong
 *
 */
public class UtilizationModelPlanetLabCPU implements UtilizationModel {

	/** The scheduling interval. */
	private	 double schedulingInterval;
	
	/** The data ( 5 min * 288 = 24 hours). */
	private final double[] data;

	
	
	
	/**
	 * Instantiates a new utilization model PlanetLab
	 * 
	 * @param inputPath the input path
	 * @param schedulingInterval the scheduling interval
	 * @throws IOException signals that an I/O exception has occurred
	 * @throws NumberFormatException  the number format exception
	 */
	public UtilizationModelPlanetLabCPU(String inputPath, double schedulingInterval)
			throws NumberFormatException, 
			IOException{
//		data = new Double[289];
//		this.setSchedulingInterval(schedulingInterval); 
//		BufferedReader input = new BufferedReader(new FileReader(inputPath));
//		int n = data.length;
//		for (int i = 0; i < n -1; i++){
//			data[i] = Integer.valueOf(input.readLine()) / 100.0;
//		}
//		
//		data[n - 1] = data[n - 2];
//		input.close();
		
		data = new double[290];
		setSchedulingInterval(schedulingInterval);
		BufferedReader input = new BufferedReader(new FileReader(inputPath));
		int n = data.length;
		for (int i = 0; i < n - 2; i++) {
			data[i] = Integer.valueOf(input.readLine()) / 100.0;
		}
		data[n - 2] = data[n - 3];
		data[n - 1] = data[n - 2];
		input.close();
	}

	/**
	 * Instantiates a new utilization model PlanetLab
	 * 
	 * @param inputPath the input path
	 * @param schedulingInterval the scheduling interval
	 * @param dataSamples number of samples in the file
	 * @throws IOException signals that an I/O exception has occurred
	 * @throws NumberFormatException  the number format exception
	 */
	public UtilizationModelPlanetLabCPU(String inputPath, double schedulingInterval, int dataSamples)
			throws NumberFormatException, 
			IOException{
		data = new double[dataSamples];
		this.setSchedulingInterval(schedulingInterval); 
		BufferedReader input = new BufferedReader(new FileReader(inputPath));
		int n = data.length;
		for (int i = 0; i < n -1; i++){
			data[i] = Integer.valueOf(input.readLine()) / 100.0;
		}
		
		data[n - 1] = data[n - 2];
		input.close();
	}

	/* (non-Javadoc)
	 * @see org.cloudbus.cloudsim.UtilizationModel#getUtilization(double)
	 */
	@Override
	public double getUtilization(double time) {
		//计算某一时刻virtual machine对cpu的利用率时，也是假设两个时间点之间的cpu利用率成线性关系。
		if (time % getSchedulingInterval() == 0) {
			return data[(int) time / (int) getSchedulingInterval()];
		}
		int time1 = (int) Math.floor(time / getSchedulingInterval());
		int time2 = (int) Math.ceil(time / getSchedulingInterval());
		double utilization1 = data[time1];
		double utilization2 = data[time2];
		double delta = (utilization2 - utilization1) / ((time2 - time1) * getSchedulingInterval());
		double utilization = utilization1 + delta * (time - time1 * getSchedulingInterval());
		return utilization;
	}

	/**
	 * Gets the scheduling interval.
	 * 
	 * @return the scheduling interval
	 */
	private double getSchedulingInterval() {
		return schedulingInterval;
	}

	/**
	 * Sets the scheduling interval.
	 * 
	 * @param schedulingInterval the new scheduling interval
	 */
	private void setSchedulingInterval(double schedulingInterval) {
		this.schedulingInterval = schedulingInterval;
	}

}
