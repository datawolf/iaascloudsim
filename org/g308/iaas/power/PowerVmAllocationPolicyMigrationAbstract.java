...
public abstract class PowerVmAllocationPolicyMigrationAbstract extends PowerVmAllocationPolicyAbstract {
	...
	public List<Map<String, Object>> optimizeAllocation(List<? extends Vm> vmList) {
		...
		//1 、 找到所有过载的物理主机
		List<MyPowerHost> overUtilizedHosts = getOverUtilizedHosts();
	...
	protected List<Map<String, Object>> getMigrationMapFromUnderUtilizedHosts(
			Collection<? extends MyPowerHost> overUtilizedHosts) {
		List<Map<String, Object>> migrationMap = new LinkedList<Map<String, Object>>();
		List<MyPowerHost> switchedOffHosts = getSwitchedOffHosts();

		// over-utilized hosts + hosts that are selected to migrate VMs to from over-utilized hosts
		//寻找欠载的物理主机时，应该除了以下几种主机：
		// A、过载的物理主机 B、已经关闭了的物理主机 C、有虚拟机迁入的物理主机
		Set<MyPowerHost> excludedHostsForFindingUnderUtilizedHost = new HashSet<MyPowerHost>();
		...
		// A、 过载的物理主机 B、关闭了物理主机
		Set<MyPowerHost> excludedHostsForFindingNewVmPlacement = new HashSet<MyPowerHost>();
		...
		while (true) {
			...
			//寻找欠载的物理主机
			MyPowerHost underUtilizedHost = getUnderUtilizedHost(excludedHostsForFindingUnderUtilizedHost);
	 ...
	 * @param overUtilizedHosts the over utilized hosts
	 */
	protected void printOverUtilizedHosts(List<MyPowerHost> overUtilizedHosts) {
		if (!Log.isDisabled()) {
			Log.printLine("Over-utilized hosts:");
			for (MyPowerHost host : overUtilizedHosts) {
	 ...
	 */
	public MyPowerHost findHostForVm(Vm vm, Set<? extends Host> excludedHosts) {
		//该函数在除了excludeHosts主机之外的其它主机中寻找合适的物理主机来放置虚拟机VM
		double minPower = Double.MAX_VALUE;
		MyPowerHost allocatedHost = null;
	 ...
	 */
	protected boolean isHostOverUtilizedAfterAllocation(MyPowerHost host, Vm vm) {
	 ...
	 * @return the list
	 */
	protected List<MyPowerHost> extractHostListFromMigrationMap(List<Map<String, Object>> migrationMap) {
		List<MyPowerHost> hosts = new LinkedList<MyPowerHost>();
		for (Map<String, Object> map : migrationMap) {
			hosts.add((MyPowerHost) map.get("host"));
		}
		return hosts;
	}
	...
	protected List<Map<String, Object>> getNewVmPlacement(
		...
		for (Vm vm : vmsToMigrate) {
			...
			MyPowerHost allocatedHost = findHostForVm(vm, excludedHosts);
	...
	protected
			List<? extends Vm>
			getVmsToMigrateFromHosts(List<MyPowerHost> overUtilizedHosts) {
	 ...
	 */
	protected List<? extends Vm> getVmsToMigrateFromUnderUtilizedHost(MyPowerHost host) {
		List<Vm> vmsToMigrate = new LinkedList<Vm>();
	 ...
	 */
	protected List<MyPowerHost> getSwitchedOffHosts() {
		List<MyPowerHost> switchedOffHosts = new LinkedList<MyPowerHost>();
		for (MyPowerHost host : this.<MyPowerHost> getHostList()) {
	...
	protected MyPowerHost getUnderUtilizedHost(Set<? extends Host> excludedHosts) {
		double minUtilization = 1;
		MyPowerHost underUtilizedHost = null;
		for (MyPowerHost host : this.<MyPowerHost> getHostList()) {
			if (excludedHosts.contains(host)) {
				continue;
			}
	 ...
	 */
	protected boolean areAllVmsMigratingOutOrAnyVmMigratingIn(MyPowerHost host) {
	 ...
	 */
	protected abstract boolean isHostOverUtilized(MyPowerHost host);
	 ...
	 */
	protected void addHistoryEntry(MyPowerHost host, double metric) {
	...
	protected void restoreAllocation() {
		...
		for (Map<String, Object> map : getSavedAllocation()) {
			Vm vm = (Vm) map.get("vm");
			MyPowerHost host = (MyPowerHost) map.get("host");
			if (!host.vmCreate(vm)) {
	 ...
	 */
	protected double getPowerAfterAllocation(MyPowerHost host, Vm vm) {
		double power = 0;
		try {
	 ...
	 * @return the power after allocation
	 */
	protected double getMaxUtilizationAfterAllocation(MyPowerHost host, Vm vm) {
	 ...
	 */
	protected double getUtilizationOfCpuMips(MyPowerHost host) {
