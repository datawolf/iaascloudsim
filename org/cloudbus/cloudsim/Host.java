...
public class Host {
	 ...
	 * 将虚拟机加入迁移，首先判断storage大小够用否，其次是内存、带宽、PE
	 * @param vm the vm
	 */
	public void addMigratingInVm(Vm vm) {
		...
		if (!getVmsMigratingIn().contains(vm)) {
			...
			getVmList().add(vm);
			//迁移完后，还需要马上进行处理，更新数据信息
			updateVmsProcessing(CloudSim.clock());//更新目标主机信息
			vm.getHost().updateVmsProcessing(CloudSim.clock());//更新源主机信息
		}
	}
