...
public class FutureQueue {
	...
	/** The sorted set. TreeSet按照所存储对象的自然序进行排序，如果不是简单对象，所存储的对象应该实现comparable接口*/
	private final SortedSet<SimEvent> sortedSet = new TreeSet<SimEvent>();
	// 代码SimEvent中实现了comparable接口。sortedSet中对象按照时间time进行排序，如果时间
	// 相等，就按照serial排序。

	/** The serial.  记录总共处理的事件数目*/
